...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await auth.login({ email, password }) as { success: boolean; error?: string };
      if (res.success) {
  ...
  return (
    <div className="flex-1 flex flex-col justify-center px-6 py-12 safe-top">
      ...
      <div className="mb-10 text-center">
        ...
        <h1 className="text-2xl font-bold text-text-primary">SplitRight</h1>
        <p className="text-text-secondary text-sm mt-1">Fair splits for everyone</p>
      </div>
